...
//扑克数据 6全部取出
const int	CGameLogic::m_bCardListData[54] =
{
...
//检查余下的牌是否能组成顺或者横
bool CGameLogic::CheckCanContinuity(int const CardIndex[MAX_INDEX],  int GhostCount)
{
	int CardIndexTemp[MAX_INDEX] = { 0 };
	memcpy(CardIndexTemp, CardIndex, sizeof(CardIndexTemp));

	int GhostCardNeeded = 0;
	for (int i = 0; i < MAX_INDEX; ++i)
	{
...
bool CGameLogic::CheckCanHu(int const HandCard[MAX_INDEX], int HandCardSize, int GhostCount)
{
	int TotalCount = HandCardSize;// +GhostCount;
	if ((TotalCount - 2) % 3 != 0 || TotalCount > MAX_HAND_COUNT)
	{
		//牌数，不满足3n+2，或大于14张
		return false;
	}
	...
	int CardIndexTemp[MAX_INDEX];
	memcpy(CardIndexTemp, HandCard, sizeof(CardIndexTemp));
	std::vector<int> PairsInfo;  //用于存放所有能构成胡牌的对子的值
	int CheckZero = ((GhostCount < 2) ? 2 : 0);//(0-未检查 1-成功 2-失败 )
