...
const int	CGameLogic::m_bCardListData[8*54] =
{
	...
	0x4E, 0x4F,// 0x4E, 0x4F, 0x4E, 0x4F, 0x4E, 0x4F, 0x4E, 0x4F, 0x4E, 0x4F, 0x4E, 0x4F, 0x4E, 0x4F,
...
//扑克字面转换索引
void CGameLogic::SwitchToCardValue(std::vector<int>& vecCard, std::vector<int>& vecCardIndex)
{
	vecCard.clear();
	for (int i = 0; i < vecCardIndex.size(); ++i)
	{
		if (vecCardIndex.at(i) <= 0) continue;
		for (int j = 0; j < vecCardIndex.at(i); j++)
			vecCard.push_back(SwitchToCardValue(i));
	}
}
//混乱扑克
