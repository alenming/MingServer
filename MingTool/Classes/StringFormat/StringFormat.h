...
public:
	...
	//传入分隔符就好 
	static void parseCsvStringVecByChar(const string strSrc, vector<int>& vecDst, char separetor);

	//处理对应类型数据模板
	template<typename T>
	static void parseCsvMathVec(const string strSrc, vector<T>& vecDst, char separetor)
	{
		if (strSrc.empty())
		{
			return;
