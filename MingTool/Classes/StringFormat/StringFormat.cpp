...
void StringFormat::parseCsvStringVecByChar(const string strSrc, vector<int>& vecDst, char separetor)
{
	if (strSrc.empty())
	{
		return;
...
std::wstring StringFormat::StringUtf8ToWideChar(const std::string& strUtf8)
{
	...
	if (!strUtf8.empty())
	{
		...
		{
			MINGLOG("Wrong convert to WideChar code:0x%x", GetLastError());
		}
	}
	return ret;
}
...
std::string StringFormat::StringWideCharToUtf8(const std::wstring& strWideChar)
{
	...
	if (!strWideChar.empty())
	{
		...
		{
			MINGLOG("Wrong convert to Utf8 code:0x%x", GetLastError());
		}
	}
	return ret;
}
...
std::string StringFormat::UTF8StringToMultiByte(const std::string& strUtf8)
{
	...
	if (!strUtf8.empty())
	{
		...
		{
			MINGLOG("Wrong convert to Ansi code:0x%x", GetLastError());
		}
	}
