...
#define DATA "Data"
string default_data = "<?xml version=\"1.0\" encoding=\"utf-8\" ?> <LocalDefault> <Data  /> </LocalDefault>";

LocalDefault* LocalDefault::_userDefault = nullptr;
string LocalDefault::_filePath = string("");
...
{
	MING_SAFE_DELETE(_userDefault);
}

void LocalDefault::setIntegerForKeyValue(const string nKey, int value) 
{
	char szValue[128];
...
//存储
void LocalDefault::setBoolForKeyValue(const string nKey, bool value)
{
	std::string Key = EncryptData(nKey.c_str());
...
void LocalDefault::setStringForKeyValue(const string nKey, const std::string & sValue)
{
	std::string Key = EncryptData(nKey.c_str());
...
void LocalDefault::setFloatForKeyValue(const string nKey, float value)
{
	char szValue[128];
	memset(szValue, 0, 128);
...
void LocalDefault::setDoubleForKeyValue(const string nKey, double value)
{
	char szValue[128];
	memset(szValue, 0, 128);
	//保留九点小数
...
void LocalDefault::setDataForKeyValue(const char* nKey, const Data& value)
{
...
//读取
int LocalDefault::getIntegerForKeyValue(const string nKey)
{
	std::string Key = EncryptData(nKey.c_str());

	string s = getValueForKey(Key.c_str());
	string parseKey = CrevasseData(s.c_str());
	return (parseKey.length() != 0 ? atoi(parseKey.c_str()) : 0);
}

bool LocalDefault::getBoolForKeyValue(const string nKey)
{
	std::string Key = EncryptData(nKey.c_str());
...
std::string LocalDefault::getStringForKeyValue(const string nKey)
{
	std::string Key = EncryptData(nKey.c_str());
	string s = getValueForKey(Key.c_str());
	string parseKey = CrevasseData(s.c_str());
...
float LocalDefault::getFloatForKeyValue(const string nKey)
{
	std::string Key = EncryptData(nKey.c_str());
	string s = getValueForKey(Key.c_str());
	string parseKey = CrevasseData(s.c_str());
...
double LocalDefault::getDoubleForKeyValue(const string nKey)
{
	std::string Key = EncryptData(nKey.c_str());
	string s = getValueForKey(Key.c_str());
	string parseKey = CrevasseData(s.c_str());
...
Data LocalDefault::getDataForKeyValue(const char* nKey)
{
	std::string Key = EncryptData(nKey);
	string result = getValueForKey(Key.c_str());
...
string LocalDefault::getValueForKey(const char* pKey)
{
	string result = "";
...
//解开密文
string LocalDefault::CrevasseData(const char* szEncryPass) const
{
	if (_mIsEn == ENCRYPT_TYPE::NO_EN)
	{
		return string(szEncryPass);
	}
	if ((szEncryPass != NULL) && (strlen(szEncryPass) >= ENCRYPT_KEY_LENGTH * 4))
	{
		...
		{
			UnChar_t szKeyBuffer[3] = { (UnChar_t)szEncryPass[i * 4], (UnChar_t)szEncryPass[i * 4 + 1], 0 };
			...
			strCrevassePass += (char)((UnChar_t)uKey ^ (UnChar_t)uEncrypt);
		}
...
const string& LocalDefault::getXMLFilePath()
{
    return _filePath;
}
