...
void XML::operator=( const XML& markup )
{
...
bool XML::SetDoc( const char* szDoc )
{
	// Reset indexes
	m_iPosFree = 1;
	ResetPos();
	...
	UnInt32_t nStartSize = m_strDoc.size() / 64 + 8;
...
bool XML::Load( const char* szFileName )
{
	...
	bool bResult = false;
	string filePath = FileUtils::getInstance()->fullPathForFilename(szFileName);
...
bool XML::Save( const char* szFileName )
{
...
bool XML::FindElem( const char* szName )
{
...
bool XML::FindChildElem( const char* szName )
{
...
string XML::GetAttribName( int n ) const
{
...
bool XML::SavePos( const char* szPosName )
{
...
bool XML::RestorePos( const char* szPosName )
{
...
bool XML::GetOffsets( int& nStart, int& nEnd ) const
{
...
string XML::GetChildSubDoc() const
{
	if ( m_iPosChild )
	{
...
//[CMARKUPDEV
string XML::FindGetData( const char* szName )
{
	if ( FindElem(szName) )
...
bool XML::FindSetData( const char* szName, const char* szData, int nCDATA )
{
	if ( FindElem(szName) )
...
// Base64 methods
string XML::EncodeBase64( const unsigned char* pBuffer, int nBufferLen )
{
...
int XML::DecodeBase64( const string& strBase64, unsigned char* pBuffer, int nBufferLen )
{
...
int XML::x_ParseError( const char* szError, const char* szName )
{
	if ( szName )
	{
		char szFormat[300];
		sprintf_s( szFormat,sizeof(szFormat),szError, szName );
		m_strError = szFormat;
	}
	else
		m_strError = szError;
...
int XML::x_ParseElem( int iPosParent )
{
...
bool XML::x_FindChar( const char* szDoc, int& nChar, char c )
{
	// static function
...
bool XML::x_FindToken( XML::TokenPos& token )
{
...
string XML::x_GetToken( const XML::TokenPos& token ) const
{
	...
	if ( token.nL > token.nR )
		return "";
	return m_strDoc.substr( token.nL, (token.nR - token.nL + (((UnInt32_t)(token.nR)<m_strDoc.size())? 1:0)) );
}

int XML::x_FindElem( int iPosParent, int iPos, const char* szPath )
{
...
int XML::x_ParseNode( XML::TokenPos& token )
{
...
string XML::x_GetTagName( int iPos ) const
{
...
bool XML::x_FindAttrib( XML::TokenPos& token, const char* szAttrib ) const
{
...
string XML::x_GetAttrib( int iPos, const char* szAttrib ) const
{
	...
	if ( szAttrib && x_FindAttrib( token, szAttrib ) )
		return x_TextFromDoc( token.nL, token.nR - (((UnInt32_t)(token.nR)<m_strDoc.size())?0:1) );
	return "";
}

bool XML::x_SetAttrib( int iPos, const char* szAttrib, int nValue )
{
	// Convert integer to string and call SetChildAttrib
	char szVal[25];
	sprintf_s(szVal, sizeof(szVal), "%d", nValue);
	return x_SetAttrib( iPos, szAttrib, szVal );
}

bool XML::x_SetAttrib( int iPos, const char* szAttrib, const char* szValue )
{
...
//[CMARKUPDEV
bool XML::x_RemoveAttrib( int iPos, const char* szAttrib )
{
...
bool XML::x_CreateNode( string& strNode, int nNodeType, const char* szText )
{
...
bool XML::x_SetData( int iPos, const char* szData, int nCDATA )
{
...
string XML::x_GetData( int iPos ) const
{
	//[CMARKUPDEV
	if ( iPos == m_iPos && m_nNodeLength )
	{
	...
	if ( ! m_aPos[iPos].iElemChild && ! m_aPos[iPos].IsEmptyElement() )
	{
				...
				&& strncmp( &token.szDoc[token.nL+1], "![CDATA[", 8 ) == 0 )
		{
			...
			if ( nEndCDATA != (int)(string::npos) && nEndCDATA < m_aPos[iPos].nEndL )
			{
...
string XML::x_TextToDoc( const char* szText, bool bAttrib ) const
{
	//
	// &lt;   less than
	// &amp;  ampersand
...
string XML::x_TextFromDoc( int nLeft, int nRight ) const
{
	//
...
void XML::x_DocChange( int nLeft, int nReplace, const string& strInsert )
{
...
void XML::x_Adjust( int iPos, int nShift, bool bAfterPos )
{
...
void XML::x_LocateNew( int iPosParent, int& iPosRel, int& nOffset, int nLength, int nFlags )
{
...
bool XML::x_AddElem( const char* szName, const char* szValue, bool bInsert, bool bAddChild )
{
	if ( bAddChild )
	{
...
bool XML::x_AddSubDoc( const char* szSubDoc, bool bInsert, bool bAddChild )
{
...
//[CMARKUPDEV
void XML::x_AdjustForNode( int iPosParent, int iPos, int nShift )
{
...
bool XML::x_AddNode( int nNodeType, const char* szText, bool bInsert )
{
...
void XML::x_RemoveNode( int iPosParent, int& iPos, int& nNodeType, int& nNodeOffset, int& nNodeLength )
{
