...
#define CHECKREPLY(r) {					\
	if (NULL == r) {					\
		MINGLOG("redisReply is NULL! f=%s, l=%d\n", __FUNCTION__, __LINE__);	\
	    goto Retry;						\
    }                                   \
}
...
redisContext *RedisStorer::GetRedisContext()
{
	...
	{
		bNeedConnect = true;
		MINGLOG("m_pContext = NULL ! and rediContext will reconnect ...\n");
	}
    else if (m_pContext->err != 0)
    {
        bNeedConnect = true;
		MINGLOG("rediContext will reconnect, err=%d, erroInfo=%s\n", m_pContext->err, m_pContext->errstr);
        redisFree(m_pContext);
        m_pContext = NULL;
    }
	...
	if (bNeedConnect)
	{
		...
		{
			MINGLOG("redisContext reconnect failed!\n");
			return NULL;
		}
		MINGLOG("redisContext reconnect success!\n");
	}
...
int RedisStorer::ListPush(const std::string &key, string &str)
{
	CHECKCONTEXT(m_pContext);
