		...
		while (conf.FindElem("server"))
		{
			...
			if (dbtype == "redis")
			{
				...
				if (NULL == getRule)
				{
					...
					{
						MINGLOG("connect to redis ip=%s, port=%d success!", rule.ip.c_str(), rule.port);
					}
					else
					{
						MINGLOG("connect to redis ip=%s, port=%d failed!", rule.ip.c_str(), rule.port);
						delete redisStorer;
						continue;
					}
					rule.storer = redisStorer;
					InsertStorer(rule);
					MINGLOG("insert redis storer storageId=%d", rule.db_id);
				}
			...
			else if (dbtype == "mysql")
			{
				...
				if (NULL == getRule)
				{
					...
					if (dbStorer->Connect())
					{
						MINGLOG("connect to mysql ip=%s, port=%d success!", rule.ip.c_str(), rule.port);
					}
					else
					{
						MINGLOG("connect to mysql ip=%s, port=%d failed!", rule.ip.c_str(), rule.port);
						delete dbStorer;
						continue;
					}
					rule.storer = dbStorer;
					InsertStorer(rule);
					MINGLOG("insert mysql storer storageId=%d", rule.db_id);
				}
