...
MysqlStorer::MysqlStorer(string dbname, string user, string pwd, string addr, int port):
m_DbName(dbname),
m_DbUser(user),
m_DbPwd(pwd),
m_DbAddress(addr),
...
//设置数据库名称
void MysqlStorer::SetDbName(string db)
{
	m_DbName = db;
}

//设置操作者的帐号
void MysqlStorer::SetDbUser(string user)
{
	m_DbUser = user;
}

//设置操作者的密码
void MysqlStorer::SetDbPwd(string pwd)
{
	m_DbPwd = pwd;
}

//设置数据库服务器的地址
void MysqlStorer::SetDbAddress(string addr)
{
	m_DbAddress = addr;
}
