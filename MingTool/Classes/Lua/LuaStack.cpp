...
LuaStack* LuaStack::m_Instance = NULL;
...
int LuaStack::executeScriptFile(const char* filename)
{
	...
	std::string fullPath = FileUtils::getInstance()->fullPathForFilename(buf);
...
int LuaStack::executeGlobalFunction(const char* functionName)
{
	...
	{
		MINGLOGERROR("name '%s' does not represent a Lua function", functionName);
		lua_pop(_state, 1);
		return 0;
	}
...
int LuaStack::executeFunction(int numArgs)
{
	...
	{
		MINGLOG("value at stack [%d] is not function", functionIndex);
	...
	if (error)
	{
		if (traceback == 0)
		{
			MINGLOG("[LUA ERROR] %s", lua_tostring(_state, -1));        /* L: ... error */
...
int LuaStack::luaLoadBuffer(lua_State *L, const char *chunk, int chunkSize, const char *chunkName)
{
	...
	if (r)
	{
		...
		{
		case LUA_ERRSYNTAX:
			MINGLOGERROR("load \"%s\", error: syntax error during pre-compilation.", chunkName);
			break;

		case LUA_ERRMEM:
			MINGLOGERROR("load \"%s\", error: memory allocation error.", chunkName);
			break;

		case LUA_ERRFILE:
			MINGLOGERROR("load \"%s\", error: cannot open/read file.", chunkName);
			break;

		default:
			MINGLOGERROR("load \"%s\", error: unknown.", chunkName);
		}
	}
#endif
	// r==0 is ok
