...
void ByteBuffer::putByte(UnChar_t n){
...
void ByteBuffer::putArray(vector<UnChar_t>& a){
	putInt(a.size());
...
void ByteBuffer::getArray(vector<UnChar_t>& a){
	int size = getInt();
	a.resize(size);
...
void ByteBuffer::getArray(vector<UnChar_t>& a,int size){
	a.resize(size);
	for(int i=0;i< size;++i){
		a[i] = getByte();
	}
...
int ByteBuffer::getLength(int offset)
{
    ...
    UnChar_t* pos = new UnChar_t[4]; //memory leak
    for(int i = 0;i < 4 ;i++)
    {
...
void ByteBuffer::getAsBytes(UnChar_t* bytes)
{
    for(int i = 0 ; i < 4 ; i++){
...
int ByteBuffer::bytesToInt(UnChar_t* bytes)
{
...
UnChar_t* ByteBuffer::intToByte(int i)
{
	UnChar_t* abyte0 = new UnChar_t[4];
	abyte0[3] = (UnChar_t) (0xff & i);
	abyte0[2] = (UnChar_t) ((0xff00 & i) >> 8);
	abyte0[1] = (UnChar_t) ((0xff0000 & i) >> 16);
	abyte0[0] = (UnChar_t) ((0xff000000 & i) >> 24);
	return abyte0;
}
MING_END