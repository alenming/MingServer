...
void SocketClient::startSocket(std::string host, int port)
{
	...
	{
		MINGLOG("============SocketClient::starSocket:already connected============");
		return;
	}
	else if(mConnectState == CONNECTING)
	{
		MINGLOG("============SocketClient::starSocket:already connecting============");
		return;
	}
	else if(mConnectState == TO_CONNECT)
	{
		MINGLOG("============SocketClient::starSocket:already to connect============");
		return;
	}
	else if (mConnectState == NOT_CONNECTED)
	{
		...
		{
			MINGLOG("============SocketClient::starSocket:ip or port invalid============");
			return;
		}
	...
	if(!mThreadSendCreated)
	{
		...
		{
			MINGLOG("============SocketClient::starSocket:send thread create error!============");
			return;
		}
	...
	{
		MINGLOG("============SocketClient::starSocket:send thread already createad!============");
	}
	...
	if( !mThreadRecvCreated )
	{
		...
		{
			MINGLOG("============SocketClient::starSocket:recv thread create error!============");
		}
		else
		{
...
void SocketClient::connectSocket()
{
	...
	else if (mConnectState == TO_CONNECT)
	{
		...
		if (mSocketId == -1)
		{
			MINGLOG("============SocketClient::connectSocket:socket create error!============");
			mConnectState = NOT_CONNECTED;
			return;
		}
		...
		if (ret != 0)
		{
			MINGLOG("============SocketClient::connectSocket:ioctlsocket error!============");
			mConnectState = NOT_CONNECTED;
			return;
		}
		else
		...
		if (ret == 0)
		{
			MINGLOG("============SocketClient::connectSocket:connect success============");
		...
		else if (ret == -1) //非阻塞连接中
		{
			...
			{
				isError = true;
				MINGLOG("============SocketClient::connectSocket:network is unreachable!============");
			}
			else if (getErrno() == EHOSTUNREACH) //主机不可达
			{
				isError = true;
				MINGLOG("============SocketClient::connectSocket:network is hostdown!============");
			}
		...
		{
			MINGLOG("============SocketClient::connectSocket:connect error ret=%d============", ret);
			mConnectState = NOT_CONNECTED;
			return;
		}
	}
	else if (mConnectState == CONNECTING)
	{
		...
		if (ret == 0)
		{
			MINGLOG("============SocketClient::connectSocket:select timeout errno=%d============", getErrno());
		...
		{
			MINGLOG("============SocketClient::connectSocket:select error ret=%d============", ret);
			mConnectState = NOT_CONNECTED;
		}
		else
		{
			...
			{
				MINGLOG("============SocketClient::connectSocket:mSocketID is not in the fdwrite============");
				mConnectState = NOT_CONNECTED;
			}
		}
	}
...
void SocketClient::threadSend()
{
	while (!mSendThreadFinished)
	{
		...
		if (mConnectState == CONNECTED)
		{
			...
			else
			{
				...
				{
					MINGLOG("============SocketClient::threadSend:select error ret=%d!============", nRet);
				...
				else
				{
					...
					else
					{
						...
						MINGLOG("============SocketClient::threadSend:send message length=%d============", msg->datalength());

						//判断缓冲区是否满了
						if(msg->datalength() + mSendBuffer.getPosition() > mSendBuffer.getLimit())
						{
							MINGLOG("============SocketClient::threadSend:send buffer full error!============");
					...
					if (isBufferReady)
					{
						...
						else
						{
							...
							{
								//可恢复错误
								MINGLOG("============SocketClient::threadSend:send recovery errorno=%d============", getErrno());
							}
							else
							{
								MINGLOG("============SocketClient::threadSend:send errorno=%d!!!============", getErrno());
	...
	MINGLOG("============SocketClient::threadSend:exit============");
}
...
void SocketClient::threadReceive()
{
	...
	while (!mRecvThreadFinished)
	{
		...
		{
			MINGLOG("============SocketClient::threadReceive:select errorno=%d============", nRet);
		...
		else
		{
			...
			{
				MINGLOG("============SocketClient::threadReceive:fdexceptions============", nRet);
			...
			else if (FD_ISSET(mSocketId, &fdreads))
			{
				...
				if(mRecvBuffer.remaining() > 0) //判断接收消息缓冲区是否已满
				{
					...
					{
						MINGLOG("============SocketClient::threadReceive:recv recvedBytes=%d============", nRecvedBytes);
						MINGLOG("============SocketClient::threadReceive:recv errorno=%d============", getErrno());
					...
					{
						MINGLOG("============SocketClient::threadReceive:server close connection!!============");
						MINGLOG("============SocketClient::threadReceive:server close connection errorno=%d============", getErrno());
					...
					{
						MINGLOG("============SocketClient::threadReceive:receive bytes:%d============", nRecvedBytes);
...
// 							CCLOG("+++++++++++++++capacity:%d+++++++++++++++",mRecvBuffer.getCapacity());
							...
							if(mRecvBuffer.remaining() >=  tmpOffset + length){
								...
								MINGLOG("============SocketClient::threadReceive:receive message:commandId=%d, length=%d============"
									, ByteBuffer::bytesToInt(message->commandId), ByteBuffer::bytesToInt(message->length));
								MINGLOG("============SocketClient::threadReceive:m_receivedMessageQueue size=%d============", mReceivedMessageQueue.size());

							}
							else if(length > mRecvBuffer.getCapacity())
							{

								MINGLOG("============SocketClient::threadReceive:message.length > m_cbRecvBuf.getCapacity()!!!============");
								...
								MINGLOG("============mRecvBuffer.position = pos!!!============");
								break;
							}
						}
				...
				{
					MINGLOG("============SocketClient::threadReceive:m_cbRecvBuf.remaining()<=0!!!============");
	...
	MINGLOG("============SocketClient::threadReceive:exit============");
}
