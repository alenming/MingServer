...
void SocketCommunicator::reconnection()
{
	...
	{
		MINGLOG("============reconnection:%d============", mAutoConnectCount);
...
{
	MINGLOG("------Send Socket Message %d:%s------", nCmdID, szData);
...
void SocketCommunicator::update(float dt)
{
	...
	if (pMsg)
	{
		...
		// 派发消息;
		MINGLOG("============receive msg:id=%d============", nRcvCmdId);
		szOutData[err] = '\0';
...
{
	MINGLOG("============SocketCommunicator::clear============");
