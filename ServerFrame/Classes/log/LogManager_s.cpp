...
LogManager_s* LogManager_s::m_Instance = NULL;

LogManager_s::LogManager_s()
:m_LogLevel(0),
m_LevelOption(true),
m_TimeOption(true),
m_DateOption(true),
...
LogManager_s* LogManager_s::getInstance()
{
    if (NULL == m_Instance)
    {
        m_Instance = new LogManager_s();
    }

    return m_Instance;
}

void LogManager_s::destroy()
{
    if (NULL != m_Instance)
    {
        delete m_Instance;
...
bool LogManager_s::addHandler(int handleId, ILogHandler_s* handler)
{
    if (NULL == handler)
    {
        return false;
    ...
    map<int, ILogHandler_s*>::iterator iter = m_Handlers.find(handleId);
...
bool LogManager_s::addHandler(ILogHandler_s* handler)
{
    if (NULL == handler)
    {
        return false;
    ...
    map<int, ILogHandler_s*>::iterator iter = m_Handlers.find(handler->getHandleId());
    if (iter != m_Handlers.end())
    {
        return false;
    }
...
bool LogManager_s::removeHandler(int handleId)
{
    map<int, ILogHandler_s*>::iterator iter = m_Handlers.find(handleId);
    if (iter != m_Handlers.end())
    {
...
ILogHandler_s* LogManager_s::getHandler(int id)
{
    map<int, ILogHandler_s*>::iterator iter = m_Handlers.find(id);
    if (iter != m_Handlers.end())
    {
        return iter->second;
    }
...
void LogManager_s::clearHandler()
{
    for (map<int, ILogHandler_s*>::iterator iter = m_Handlers.begin();
...
void LogManager_s::writelog(int tag, int level, const char* file, const char* func, int line, const char* log, ...)
{
    ...
    if (level >= m_LogLevel)
    {
        ...
        map<int, ILogHandler_s*>::iterator iter = m_Handlers.begin();
...
void LogManager_s::updateFormat(int level, const char* log, const char* file, const char* func, int line)
{
    char szBuf[MAX_BUF];
    m_StringFormat = "";
...
void LogManager_s::HexDumpImp(const void *pdata, unsigned int len)
{
	{
		if (pdata == 0 || len == 0)
		{
		...
		ILogHandler_s* pHandle = getHandler(2);
		if (pHandle == NULL)
		{
			return;
		}
